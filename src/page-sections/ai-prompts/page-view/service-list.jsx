...
export default function ServiceList() {
  ...
  const handleEdit = () => {
    setNewSettings({
      ...
      revenuecat_api_key_android: settings?.revenuecat_api_key_android || "",
      revenuecat_api_key_ios: settings?.revenuecat_api_key_ios || "",
  ...
  const handleDelete = async () => {
    try {
      ...
      // For example: await deleteDoc(doc(DB, "settings", "config"));
      await fetchAISetting();
      toast.success(t("Settings deleted successfully"));
    } catch (error) {
      console.error("Error deleting settings:", error);
      toast.error(t("Failed to delete settings"));
    }
  };
...
const handleSaveSettings = async () => {
  try {
    ...
    // If it's a new doc, add createdAt; otherwise preserve the original one
    const docSnap = await getDoc(docRef);
    if (docSnap.exists()) {
      const existingData = docSnap.data();
      if (existingData?.createdAt) {
        settingsData.createdAt = existingData.createdAt;
      }
    } else {
      settingsData.createdAt = currentTime;
    }

    await updateDoc(docRef, settingsData);
    toast.success(t("Settings saved successfully"));

    await fetchAISetting(); // Refresh the UI
    setOpenAddDialog(false);
    setIsEditing(false);
  } catch (error) {
  ...
  <Card>
      ...
      >
        <DialogTitle>
          {isEditing ? t("Edit Settings") : t("Add New Settings")}
        </DialogTitle>
        <DialogContent>
          <Box sx={{ pt: 2 }}>
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} md={6}>
                <TextField
                  fullWidth
                  label={t("Android RevenueCat Key")}
                  name="revenuecat_api_key_android"
                  value={newSettings.revenuecat_api_key_android}
                  onChange={handleInputChange}
                  margin="normal"
                />
              </Grid>
              <Grid item xs={12} md={6}>
                <TextField
                  fullWidth
                  label={t("iOS RevenueCat Key")}
                  name="revenuecat_api_key_ios"
                  value={newSettings.revenuecat_api_key_ios}
                  onChange={handleInputChange}
                  margin="normal"
                />
              </Grid>
